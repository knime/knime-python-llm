...
public final class MessageValueRenderer extends DefaultDataValueRenderer
    ...
    protected void setValue(final Object value) {
        ...
        if (value instanceof MessageValue m) {

            /* ── basics ─────────────────────────────────────────────── */
            MessageType type = m.getMessageType();
            List<MessageContentPart> parts = m.getContent();
            Optional<List<ToolCall>> toolOps = m.getToolCalls();
            Optional<String> toolIdOp = m.getToolCallId();
            Optional<String> toolNameOp = m.getToolName();
            ...
            if (MessageType.AI.equals(type) && toolOps.isPresent() && !toolOps.get().isEmpty()) {
                ...
                for (ToolCall tc : toolOps.get()) {
                    toolCalls.append(div().withStyle("""
                        ...
                        .with(pre()
                            .withText((tc.arguments() == null || tc.arguments().isBlank()) ? "{}" : tc.arguments()))
                        .render());
                }
            }
            ...
            String pillBg = switch (type) {
                case USER -> "#eff1f2"; // #f4f8fb
                case AI -> "#eff1f2"; // #f4fbf5
                case TOOL -> "#eff1f2";
                default -> "#f0f0f0";
            };
            ...
            /* ── build HTML ───────────────────────────────────────── */
            ContainerTag wrapper = div().withStyle("margin-bottom:8px;") // outer wrapper
                .with(div().withStyle("""
                        min-width:20%;
                        max-width:80%;
                        width:100%;
