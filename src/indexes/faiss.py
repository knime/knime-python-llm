...
class FAISSVectorstorePortObjectContent(VectorStorePortObjectContent):
    ...
    def __init__(self, spec: knext.PortObjectSpec, embeddings_port_object: EmbeddingsPortObject) -> None:
...
@knext.input_port(
    "Embeddings", "The embeddings model to use for the vector store.", embeddings_port_type
)
@knext.input_table(
    name="Documents",
    description="""Table containing a string column representing documents that will be used in the vector store.""",
)
@knext.output_port("FAISS Vector Store", "The created FAISS vector store.", vector_store_port_type)
...
@knext.output_port("FAISS Vector Store", "The loaded vector store.", vector_store_port_type)
class FAISSVectorStoreLoader:
    ...
    def execute(
        ...
        return VectorStorePortObject(
            spec=VectorStorePortObjectSpec(self.create_spec_content()),
            content=FAISSVectorstorePortObjectContent(self.create_spec_content(), embeddings_port_object)
        )
...
@knext.input_table("Queries", "Table containing a string column with the queries for the vector store.")
@knext.output_table("Result table", "Table containing the queries and their closest match from the db.")
