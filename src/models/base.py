...
class OutputModeOptions(knext.EnumParameterOptions):
    ...
    JSON = (
        ...
        that parse into valid JSON object. This option always instructs the model 
        to produce a JSON output via some message in the conversation, for example via your system message.
...
def _get_response_format_value_switch() -> knext.EnumParameter:
    return knext.EnumParameter(
        "Output mode",
        "Choose between different output modes.",
        OutputModeOptions.Text.name,
        OutputModeOptions,
