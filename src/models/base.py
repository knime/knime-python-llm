...
class OutputModeOptions(knext.EnumParameterOptions):
    JsonMode = (
        "JSON Mode",
        """
        When JSON mode is selected, the model is constrained to only generate strings 
        ...
        to see which models currently support JSON mode.
        """,
    )
    Default = (
        "Default",
...
def _get_response_format_value_switch() -> knext.EnumParameter:
    return knext.EnumParameter(
        "Output Settings",
        "Choose between different output structures.",
        OutputModeOptions.Default.name,
        OutputModeOptions,
        style=knext.EnumParameter.Style.VALUE_SWITCH,
        since_version="5.4.0",
    ).rule(
        knext.DialogContextCondition(_is_valid_model_type),
        knext.Effect.SHOW,
    )
...
class LLMPrompter:
    ...
    **Note**: If you use the
    ...
    response_format = _get_response_format_value_switch()
    ...
    def execute(
        ...
        if self.response_format == OutputModeOptions.Default.name:
