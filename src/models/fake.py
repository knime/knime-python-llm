...
class FakeDictLLM(LLM):
    ...
    def handle_missing_value(self, prompt: str):
        if self.option == MissingValueHandlingOptions.Fail.name:
            raise knext.InvalidParametersError(
                f"Could not find matching answer for prompt: '{prompt}'. Please make sure, that the prompt exactly matches the provided answers."
            )
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        return self.response_dict.get(prompt, self.handle_missing_value(prompt))
    ...
    async def _acall(
        ...
        **kwargs: Any,
        ...
        return self.response_dict.get(prompt, self.handle_missing_value())
