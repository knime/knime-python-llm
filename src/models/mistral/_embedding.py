...
from ._util import MISTRAL_EMBEDDING_MODELS_FALLBACK
...
class MistralEmbeddingModelConnector:
    ...
    the curated list of known embedding models, or switch to the complete list to select
    from all models available for your API key.

    If Mistral AI releases a new embedding model that is not contained in the predefined list,
    you can select it using the **All models** option.
    ...
    **Note**: If you use the
    ...
    model = knext.StringParameter(
        "Model",
        "Select a Mistral AI embedding model.",
        default_value=MISTRAL_EMBEDDING_MODELS_FALLBACK[0],  # mistral-embed
