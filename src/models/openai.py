...
class GPTImage1Settings:
    ...
    class QualityOptions(knext.EnumParameterOptions):
        High = ("High", "Generates images with the highest level of detail and consistency across the entire image.")
        Medium = ("Medium", "Balances image quality and generation speed for general-purpose use.")
        Low = ("Low", "Produces lower quality images with faster generation time and reduced computational load.")
    ...
    class BackgroundOptions(knext.EnumParameterOptions):
        ...
        Auto = ("Auto", "Automatically selects the most appropriate background type based on the image content.")
...
class ImageModels(knext.EnumParameterOptions):
    GPT_IMAGE_1 = ("GPT Image 1", "State-of-the-art quality, excels at detailed and complex prompts (recommended).")
    DALL_E_3 = ("DALL-E 3", "Previous-generation model: faster outputs with solid everyday quality.")
...
@knext.input_table(
    "Image table", "Table containing the images to edit.", optional=True
)
...
class OpenAIDALLEView:
    ...
    **Note**: If you use the
    ...
    def configure(
        ...
        if not self.prompt:
            raise knext.InvalidParametersError("The image generation prompt cannot be empty.")
    ...
    def execute(
        ...
        if self.model == ImageModels.DALL_E_3.name:
            ...
            if has_image_table:
                images = self._prepare_images(image_table, self.gpt_image_1_settings.image_columns)
